function calculatePension() {
    ...
    const baseSalary = parseFloat(degreeLevel);
    const averageBaseSalary = parseFloat(degreeLevel);
  ...
  /*   if (!averageBaseSalaryInput.value || isNaN(averageBaseSalary) || averageBaseSalary <= 0) {
        ...
        if (document.getElementById('degree').value=="0") {
            alert('يرجى تحديد الدرجة الوظيفية.');
            degree.focus();
            return;
         }
    ...
    document.getElementById('baseSalary1').textContent = ` الراتب الاسمي للدرجة الوظيفية: ${Math.floor(baseSalary).toLocaleString()} دينار`;
